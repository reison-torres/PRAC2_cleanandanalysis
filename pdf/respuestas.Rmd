...
titanic.data <- titanic.data[,!(colnames(titanic.data) %in% c("PassengerId","Name"))]
...
```{r echo=TRUE, message=FALSE, warning=FALSE}
data.frame(Total=sort(colSums(titanic.data == ""), decreasing = TRUE),Porcentaje = sort(round(colMeans(titanic.data == "")*100, digits = 2), decreasing = TRUE))["Cabin",]
...
Como podemos observar las variables __SibSp__, __Parch__ y __Fare__, presenta datos con valores igual a cero, pero para las variables __SibSp__, __Parch__ este valor cero significa que no tiene no tienen familiares abordo.
...
# Proporcion en 
data.frame(Var = c("Fare"),Total = length(titanic.data$Fare[titanic.data$Fare == 0 & !is.na(titanic.data$Fare)]),Porcentaje = round((length(titanic.data$Fare[titanic.data$Fare == 0 & !is.na(titanic.data$Fare)])/dim(titanic.data)[1])*100,2))
